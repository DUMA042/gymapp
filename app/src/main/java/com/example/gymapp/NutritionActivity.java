...
public class NutritionActivity extends AppCompatActivity implements CustomAdapter.ForecastAdapterOnClickHandler {
    private RecyclerView mRecyclerView;
    ArrayList<DataClass> hold_data=new ArrayList<>();
    ArrayList<DataClass> hold_data_vegan=new ArrayList<>();
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        String check = intentThatStartedThisActivity.getStringExtra(Intent.EXTRA_TEXT);
        if(check.equals("1")){
            mCustomAdapter= new CustomAdapter(hold_data_vegan,this);
            mRecyclerView.setAdapter(mCustomAdapter);
            mCustomAdapter.setWeatherData(hold_data_vegan);
        }
        else{
    ...
    void dosetup(){
        ...
        values5.put(gymContract.nutritionEntry.COLUMN_Nutrition_NAME,"Beef stew");
        values5.put(gymContract.nutritionEntry.COLUMN_Nutrition_Calories,"600 gm");
        values5.put(gymContract.nutritionEntry.COLUMN_Nutrition_Vegan, 0);
        values5.put(gymContract.nutritionEntry.COLUMN_Nutrition_Protein, 45);
         db.insert(gymContract.nutritionEntry.TABLE_NAME, null, values3);
        db.insert(gymContract.nutritionEntry.TABLE_NAME, null, values4);
            db.insert(gymContract.nutritionEntry.TABLE_NAME, null, values5);
        }
        cursor3.close();
    }
    ...
    void doinsertdata(){
        ...
        try {
            ...
            while (cursor3.moveToNext()) {
                ...
                if (currentvegan==0){
                    hold_data_vegan.add(data_object);
                }
            }
        }finally {
            cursor3.close();
    ...
    public void onClick(String item) {
        Toast.makeText(this, "The Nutrition has " + item+" been has been saved ", Toast.LENGTH_SHORT).show();
